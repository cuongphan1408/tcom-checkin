...
class SurveillanceSystem(object):
   ...
   def __init__(self):
        ...
        #   thread.start()
   def add_camera_check(self, camera):
   ...
   def remove_camera(self, camID):
        ...
        ID = int(camID[7:])
        #THSS = threading.currentThread()
        c1 = self.cameras[ID]     
        c1.video.release()   
        # t1 = self.cameraProcessingThreads[ID] 
        # threading._active.pop(c1.captureThread.ident) 
        # threading._active.pop(t1.ident)
        # kill the process!        
        self.cameras.pop(ID)
        self.cameraProcessingThreads.pop(ID)
        ...
        # else:
        #     break
   def process_checkin(self,camera):
        ...
        while not stop:
            ...
            peopleFound = False #khong detect duoc mat nguoi
            ...
            for face_bb in camera.faceBoxes:       
                if camera.dlibDetection == False:
                        x, y, w, h = face_bb
                        face_bb = dlib.rectangle(x, y, x+w, y+h)
                ...
                #cv2.imwrite('imagechecked/checkimgprt'+str(count)+ person.time +'.png',checkimgprt)
                #cv2.imwrite('imagechecked/frame'+ str(count)+person.time +'.png',frame)
                #cv2.imwrite('imagechecked/alignedFace'+ str(count)+ person.time +'.png',alignedFace) 
                now = datetime.now()
                now_time = now.strftime("%H%M%S%d%m%Y")    
                person.time = now_time           
                return person,frame
            if end < time.time():
                timeoutObj = None
                return timeoutObj,frame
            

   def logdata(self,log_process,pid,log_message,log_image):
        
        #log to text file
        path_txt = "logdata/" + pid + "/"
        name_txt = log_process + ".txt"        
        if not os.path.exists(path_txt):
            print( "Creating New Face Dircectory: " + path_txt + name_txt)
            os.makedirs(path_txt)
        f=open(path_txt+name_txt, "a+")
        f.write(log_message +"\n")
        f.close() 

        #save image file
        path_img = "logdata/" + pid + "/" + log_process + "/"
        name_img = log_message + ".jpg"
        if not os.path.exists(path_img):
            print( "Creating New Image Dircectory: " + path_img + name_img)
            os.makedirs(path_img)
        cv2.imwrite(path_img + name_img,log_image)
        #cv2.imwrite("path_img+name_img[:-10] + str(log_image.size)",log_image)
   ...
   def process_check(self,camera):
        ...
        while not stop:
            ...
            #print("---camera.tempFrame = frame---")

            #################################################################
            ########camera.cameraFunction == "detect_recognise_track"########
            #################################################################
            if camera.cameraFunction == "detect_recognise_track":
                ...
                for x, y, w, h in peopleRects:
                    #print("--- for x, y, w, h in peopleRects: ---")
                    peopleFound = True
                    person_bb = dlib.rectangle(x, y, x+w, y+h) 
                    personimg = ImageUtils.crop(frame, person_bb, dlibRect = True)   # Crop regions of interest 
                    checkimgprt = personimg
                    ...
                    for i in range(len(camera.trackers) - 1, -1, -1): 
                        ...
                        if camera.trackers[i].overlap(person_bb):
                           ...
                           for face_bb in camera.faceBoxes: 
                                ...
                                print("make_prediction-----------------------------line--479")
                                ...
                                cv2.imwrite('imagechecked/checkimgprt'+ camera.trackers[i].person.time +'.png',checkimgprt)
                                cv2.imwrite('imagechecked/personimg'+ camera.trackers[i].person.time +'.png',personimg)
                                return camera.trackers[i].person
                                ...
                                if len(camera.faceBoxes) == 1:
                                    ...
                                    xxx = camera.trackers[i].person
                                    xxy = predictedName
                                    yyy = self.recogniser.getSquaredl2Distance(camera.trackers[i].person.rep ,predictions['rep'])
                                            ...
                                            with camera.peopleDictLock:
                                                    for ID, person in camera.people.items():  # iterate through all detected people in camera
                                                        ...
                                                        if person.identity== predictedName or self.recogniser.getSquaredl2Distance(person.rep ,predictions['rep']) < 0.8:
                                                                ...
                                                                logger.info( "====> New Tracker for " +person.identity[0]+ " <===")
                                    ...
                                    listFaceAligned = camera.trackers[i].person
                                    detectProcess = True
                        ...
                        for face_bb in camera.faceBoxes:       
                            if camera.dlibDetection == False:
                                  ...
                                  if len(camera.faceDetector.detect_cascadeface_accurate(faceimg)) == 0:
                                        continue                            
                            predictions, alignedFace =  self.recogniser.make_prediction(personimg,face_bb)
                            print("make_prediction-----------------------------line--572")
                            alreadyBeenDetected = False
                            with camera.peopleDictLock:
                            ...
                            if not alreadyBeenDetected:
                                    ...
                                    logger.info( "====> New Tracker for new person <=")
                                    camera.trackers.append(Tracker(frame, person_bb, person,strID))
                                    cv2.imwrite('imagechecked/checkimgprt'+ person.time +'.png',checkimgprt)
                                    cv2.imwrite('imagechecked/personimg'+ person.time +'.png',personimg)
                                    return person
   ...
   def process_frame(self,camera):
        ...
        while not stop:
            ...
            #  print(type(frame))
            #  print("===========================")
            #  print(type(camera.tempFrame))
             #if frame == None:  # Checks to see if the new frame is the same as the previous frame
             if frame is None:
                 continue
             frame = ImageUtils.resize(frame)
             height, width, channels = frame.shape
            ...
            # Frame rate calculation 
             ...
             ##################################################################################################################################################
                    ...
                    for face_bb in camera.faceBoxes: 
                        ...
                        predictions, alignedFace = self.recogniser.make_prediction(frame,face_bb) 
                        print("make_prediction-----------------------------line--261")
                        with camera.peopleDictLock:
                 ...
                 if state == 1: # If no faces have been found or there has been no movement
                    ...
                    if len(camera.faceBoxes) == 0:
                        ...
                        for face_bb in camera.faceBoxes: 
                            ...
                            predictions, alignedFace = self.recogniser.make_prediction(frame,face_bb)
                            print("make_prediction-----------------------------line--335")
                            with camera.peopleDictLock:
                              if camera.people.has_key(predictions['name']):
                                  if camera.people[predictions['name']].confidence < predictions['confidence']:
                                      camera.people[predictions['name']].confidence = predictions['confidence']
              ...
              ####################################################################################################################################################################
                    ...
                    for x, y, w, h in peopleRects:
                        ...
                        for face_bb in camera.faceBoxes: 
                              ...
                              predictions, alignedFace = self.recogniser.make_prediction(personimg,face_bb)
                              print("make_prediction-----------------------------line--401")
                              with camera.peopleDictLock:
                                if camera.people.has_key(predictions['name']):
                                    if camera.people[predictions['name']].confidence < predictions['confidence']:
                                        camera.people[predictions['name']].confidence = predictions['confidence']
              ...
              #############################################################################################################################################################################
                        ...
                        if camera.trackers[i].overlap(person_bb):
                           ...
                           for face_bb in camera.faceBoxes: 
                                ...
                                if len(camera.faceBoxes) == 1:
                                    ...
                                    # l2Distance is between 0-4 Openface found that 0.99 was the average cutoff between the same and different faces
                                    # the same face having a distance less than 0.99 
                                    xxx = camera.trackers[i].person.identity
                                    xxy = predictedName
                                    yyy = self.recogniser.getSquaredl2Distance(camera.trackers[i].person.rep ,predictions['rep'])
                                    if self.recogniser.getSquaredl2Distance(camera.trackers[i].person.rep ,predictions['rep']) > 0.99 and (camera.trackers[i].person.identity!= predictedName): 
                ...
                for i in range(len(camera.trackers) - 1, -1, -1): # starts with the most recently initiated tracker
                    if self.drawing == True:
                          ...
                          xxxx1 = frame
                          xxxx2 = camera.trackers[i].person.identity[0]
                          xxxx3 = camera.trackers[i].person.confidence
                          xxxx4 = camera.trackers[i].bb.left()
                          xxxx5 = camera.trackers[i].bb.top()
                          cv2.putText(frame, camera.trackers[i].person.identity[0]+ " " + str(camera.trackers[i].person.confidence)+ "%", (camera.trackers[i].bb.left(), camera.trackers[i].bb.top() - 10),
             ...
             # Used for testing puposes
                    ...
                    for face_bb in camera.faceBoxes: 
                        ...
                        with self.testingResultsLock:
                            ...
                            predictions, alignedFace = self.recogniser.make_prediction(frame,face_bb)
                            print("make_prediction-----------------------------line--655")
             ...
             # not fully implmented - was mainly used for face capture during testing
                    ...
                    for face_bb in camera.faceBoxes: 
                        ...
                        with self.testingResultsLock:
                            ...
                            predictions, alignedFace = self.recogniser.make_prediction(frame,face_bb)
                            print("make_prediction-----------------------------line--689")
