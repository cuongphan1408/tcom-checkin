...
def testupload():
    ...
    if request.method == 'POST':
        ...
        s = json.dumps(data1, indent=4, sort_keys=True)
        print(s)
        value = data.read()
...
def checkin():
    ...
    if request.method == 'POST':  
        ...
        ##--------------ADD CAMERA--------------##
        #camURL = request.json['url']
        camURL = request.form.get('camURL')  
        print(request)
        print("URRRLLLL:", camURL)
        
                
        #(camURL,application,detectionMethod,fpsTweak))
        try:
            camera = SurveillanceSystem.Camera.IPCamera(camURL,'detect_recognise_track','true','false')   
        except:
            print("error")     
            return
        render_template('index.html')   
        print("read camera cuccess-----------------------")
        ...
        checkNone = False
        for i in range(3):
            checkPerson,log_image = HomeSurveillance.process_checkin(camera) 
                        
                        #cv2.imwrite('imagechecked/' + checkPerson.identity[0] + '_' + str(checkPerson.confidence) + str(i) + '_' + checkPerson.time + '.png',checkPerson.face)
            if checkPerson != None: #neu khong timeout               
                print("xxxxxxxxxxxxxpeoplexxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",checkPerson.identity[0],checkPerson.confidence)
                if i == 0 or checkNone == True:
                    person = checkPerson
                
                if checkPerson.confidence > person.confidence:
                    person = checkPerson    
                if person.confidence > 80:
                    break
            else: #neu timeout
                if checkNone == True: #neu 2 lan deu timeout
                    break
                checkNone = True
        ...
        if person == {}:
            ...
            pid = person.identity[0]
            time = person.time #Monday 23 September 2019 02:27:41PM
        ...
        if accuracy > 40:
            ...
            code = 201
            message = "don't recognize person"
        else:
            code = 404
            message = "timeout"
        print("---DONE---")    
        ...
        #imgplot = plt.imshow(person.face)
        #plt.show()
        ...
        #log info to file
        log_process = "checkin"
        log_message = pid + " " + log_process + " " + str(accuracy) + " " + time
        if log_image.all() != {}:
            HomeSurveillance.logdata(log_process,pid,log_message,log_image)            
            print("write log file success")
        return jsonify(data)
...
def check_out():
    if request.method == 'POST':  
        ...
        camURL = request.form.get('camURL')        
        #(camURL,application,detectionMethod,fpsTweak))
        camera = SurveillanceSystem.Camera.IPCamera(camURL,'detect_recognise_track','false','false')        
        render_template('index.html')   

        ##--------------PROCESS FRAME--------------##
        person = HomeSurveillance.process_check(camera)            
        pid = person.identity[0]
        time = person.time #Monday 23 September 2019 02:27:41PM
        accuracy = person.confidence
        if accuracy > 80:
            ...
            for i, camera in enumerate(HomeSurveillance.cameras):
                with HomeSurveillance.cameras[i].peopleDictLock:
                    for key, person in camera.people.items():  
                        persondict = {'identity': key , 'confidence': person.confidence, 'camera': i, 'timeD':person.time, 'prediction': person.identity[0],'thumbnailNum': len(person.thumbnails)}
